...
myapp.get("/StudentHomepage", async (req, res) => {
  ...
  console.log('check data for StudentHomepage', studentData);
});
...
myapp.get("/studentAppointmentStatus", async (req, res) => {
  try {
    ...
    res.render("studentAppointmentStatus", {
      studentData,
      pendingAppointment,
      acceptedAppointment,
    });
...
myapp.get("/studentAppointmentHistory", async (req, res) => {
  try {
    ...
    res.render("studentAppointmentHistory", {
      studentData,
      appointmentHistory,
      ignoredAppointmentHistory,
    });
  } catch (error) {
    // Handle any unexpected server errors
...
myapp.get("/CounselorHomePage", async (req, res) => {
  ...
  try {
    ...
    const { data: newAppointments, error: newAppointmentsError } =
      await supabase
    ...
    if (newAppointmentsError) {
      console.error(
        "Error fetching new appointments:",
        newAppointmentsError.message
      );
    ...
    for (const appointment of pendingAppointments) {
      const appointedDateTime = new Date(
        `${appointment.appointed_date} ${appointment.appointed_time}`
      );
      ...
      if (currentTime > appointedDateTime) {
        ...
        if (insertError) {
          console.error(
            "Error inserting rejected appointment:",
            insertError.message
          );
        ...
        if (deleteError) {
          console.error(
            "Error deleting expired appointment:",
            deleteError.message
          );
...
myapp.get("/CounselorAcceptedAppointmentPage", async (req, res) => {
  try {
    ...
    res.render("CounselorAcceptedAppointmentPage", {
      counselorData,
...
myapp.get("/CounselorAppointmentHistoryPage", async (req, res) => {
  try {
    ...
    res.render("CounselorAppointmentHistoryPage", {
      counselorData,
      appointmentHistory,
    });
  } catch (error) {
...
myapp.get("/CounselorLogs", async (req, res) => {
  try {
    ...
    const { data: counselorLog, error } = await supabase
      ...
      .select("*")
      .eq("counselor_email", counselorEmail)
      .order("date_encoded", { ascending: true }, "time_encoded", {
        ascending: true,
      });
...
myapp.get("/adminViewAccounts", async (req, res) => {
  try {
    ...
    res.render("adminViewAccounts", {
      studentViewAccounts,
      counselorViewAccounts,
    });
  } catch (error) {
    // Handle any unexpected server errors
    console.error("Server error:", error.message);
...
myapp.post("/register", async (req, res) => {
  ...
  try {
    ...
    const emailExists = await supabase
      .from(
        accountType === "Student" ? "Student Accounts" : "Counselor Accounts"
      )
      .select("email")
    ...
    if (emailExists.data) {
      ...
      res.status(400).json({
        error:
          "The email is already registered. Please log in or reset your password if you forgot it.",
      });
      return;
    }
...
// Middleware to manually save student or counselor data in the session
// Middleware to manually save student or counselor data in the session
myapp.use('/login', async (req, res, next) => {
  ...
  try {
    ...
    console.log('Counselor Data:', counselorData); // Log the counselor data
...
myapp.post('/login', (req, res) => {
  ...
  if (req.session.studentData) {
    accountType = 'Student';
    userData = req.session.studentData;
  } else if (req.session.counselorData) {
    accountType = 'Counselor';
    userData = req.session.counselorData;
  } else {
...
myapp.post("/create-appointment", async (req, res) => {
  try {
    ...
    const appointmentTimeStr = appointmentDateTime.toLocaleString(
      "en-US",
      options
    );

    // Extract the date component
    const appointmentDateStr = appointmentDateTime.toLocaleDateString("en-US", {
      timeZone: "Asia/Manila",
    });

    const { data, error } = await supabase.from("Pending Appointment").upsert([
      {
        email: userEmail,
...
myapp.post("/studentCancelAppointment/:appointmentId", async (req, res) => {
  try {
    ...
    if (insertError) {
      console.error(
        "Error inserting cancelled appointment:",
        insertError.message
      );
    ...
    if (deleteError) {
      console.error(
        "Error deleting appointment from pending:",
        deleteError.message
      );
...
myapp.post("/updateDepartments", async (req, res) => {
  ...
  try {
    ...
    const departmentsToDelete = existingDepartmentNames.filter(
      (dept) => !departments.includes(dept)
    );
    const departmentsToInsert = departments.filter(
      (dept) => !existingDepartmentNames.includes(dept)
    );
    ...
    await supabase
      .from("Counselor Role")
      .insert(departmentsToInsert.map((dept) => ({ email, department: dept })));
...
myapp.post("/acceptAppointment/:appointmentId", async (req, res) => {
  try {
    ...
    if (insertError) {
      console.error(
        "Error inserting accepted appointment:",
        insertError.message
      );
...
myapp.post("/completeAppointment/:appointmentId", async (req, res) => {
  try {
    ...
    if (insertError) {
      console.error(
        "Error inserting completed appointment:",
        insertError.message
      );
      return res.status(500).send("Failed to cancel the appointment");
    }
    ...
    if (deleteError) {
      console.error(
        "Error completed appointment from pending:",
        deleteError.message
      );
      // Handle the error (appointment accepted but not removed from pending)
    }
...
myapp.post("/counselorEncoding", async (req, res) => {
  ...
  try {
    ...
    const encodedDateStr = encodedDateTime.toLocaleDateString("en-US", {
      timeZone: "Asia/Manila",
    });
